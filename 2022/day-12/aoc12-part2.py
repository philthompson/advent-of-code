...
shortest_route_length_from_any_start = -1
results = {}

# start from any position on the grid at height "a"
for x in range(0, size_x):
	for y in range(0, size_y):
		...
		# this doesn't need to be reset, probably, but i'll do
		#   it here anyway
		recursion_save_points = []

		# reset this for each starting point
		shortest_route_length = -1

		take_all_possible_steps(x, y, [(x, y)], 0)
		...
		if shortest_route_length_from_any_start < 0 or shortest_route_length < shortest_route_length_from_any_start:
			shortest_route_length_from_any_start = shortest_route_length

		results["({},{})".format(x, y)] = shortest_route_length

		print("starting from ({},{}), shortest route length is: {}".format(x, y, shortest_route_length))


print("all results:")
print(results)
print("overall shortest route from any starting position: [{}]".format(shortest_route_length_from_any_start))