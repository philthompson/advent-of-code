...
class NumVal:
	...
	def do_moves(self):
		global list_length
		# since we do not count the moving node itself while
		#   moving it, we take modulo the length of list minus 1
		mod_mix_number = self.value % (list_length - 1)
		if self.value == 0:
			return
		elif mod_mix_number > 0:
			cursor = self
			i = 0
			while i < mod_mix_number:
...
for line in sys.stdin:
	...
	the_val = NumVal(int(line) * 811589153, prev_val, -1)
...
for i in range(0, 10):
	print("performing mix round {}".format(i+1))
